...
class SentenceBox(object):
    ...
    def make_boundingbox(self, hm_wd, hm_pos,
                         min_bound=12, resize_val=1.1,
                         aspect_val=1.25, dup_threathold=0.033):
    ...
    def make_detectionscore(self, hm_wd_all):
        for i in range(len(self.boundingboxs)):
            x1, y1, x2, y2 = self.boundingboxs[i].x1, self.boundingboxs[
                i].y1, self.boundingboxs[i].x2, self.boundingboxs[i].y2
            y_pred = hm_wd_all[y1:y2, x1:x2]
            w, h = x2-x1, y2-y1
            y_true = (np.exp(
                -(((np.arange(w)-(w/2))/(w/10))**2)/2
            )).reshape(1, -1) * (np.exp(
                -(((np.arange(h)-(h/2))/(h/10))**2)/2
            )).reshape(-1, 1)
            self.boundingboxs[i].detectionscore = 1.0 - \
...
class CenterLine:
    def __init__(self, hm_sent, hm_word, hm_pos, p1, p2):
        if hm_sent.shape == hm_word.shape and \
                hm_sent.shape[0:2] == hm_pos.shape[1:3]:
        ...
        for x in range(self.p1[0], self.p2[0]+dx, dx):
            for y in range(self.p1[1], self.p2[1]+dy, dy):
                m = max(self.hm_pos[0][y][x]*self.hm_pos.shape[0],
                        self.hm_pos[1][y][x]*self.hm_pos.shape[1])
    ...
    def _intersect(self, p1, p2, p3, p4):
        tc1 = float(p1[0] - p2[0]) * (p3[1] - p1[1]) + \
            (p1[1] - p2[1]) * (p1[0] - p3[0])
        tc2 = float(p1[0] - p2[0]) * (p4[1] - p1[1]) + \
            (p1[1] - p2[1]) * (p1[0] - p4[0])
        td1 = float(p3[0] - p4[0]) * (p1[1] - p3[1]) + \
            (p3[1] - p4[1]) * (p3[0] - p1[0])
        td2 = float(p3[0] - p4[0]) * (p2[1] - p3[1]) + \
            (p3[1] - p4[1]) * (p3[0] - p2[0])
    ...
    def _distance(self, p1, p2, other):
        ...
        if any([i == (0, 0) for i in (ab, ap, bp)]):
            return 0
        d = np.cross(ab, ap)
        l = np.sqrt(ab[0]**2+ab[1]**2)  # noqa
        e = d / l  # 点と線の距離
        v = [ab[0]/l, ab[1]/l]
        m = np.dot(v, ap)
        if m >= 0 and m <= l:
    ...
    def _score(self):
        ...
        filt = cv2.line(filt, tuple(self.p1), tuple(
            self.p2), (255, 255, 255), self.maxw)
        return np.sum((self.hm_word > 0.015)[filt != 0])

    def dist(self, other):
        if self._intersect(self.p1, self.p2, other.p1, other.p2):
            return 0
        d1 = min(self._distance(self.p1, self.p2, other.p1),
                 self._distance(self.p1, self.p2, other.p2))
        d2 = min(self._distance(other.p1, other.p2, self.p1),
                 self._distance(other.p1, other.p2, self.p2))
        return min(d1, d2)
    ...
    def contain(self, other):
        if self._intersect(self.p1, self.p2, other.p1, other.p2):
            return True
        d = max(self._distance(self.p1, self.p2, other.p1),
                self._distance(self.p1, self.p2, other.p2))
        return d < self.maxw
